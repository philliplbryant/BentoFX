...
1. Add version/schema metadata to persisted layouts.
2. Reduce mapper/DTO boilerplate with records, MapStruct, Immutables, or Jackson configuration where appropriate.
3. Improve lifecycle management for auto-save and provider-discovered resources.
4. Split `DockingLayoutSaver` and `DockingLayoutRestorer` into smaller collaborators.
5. Remove implicit fallback identifiers based on `System.identityHashCode`.
