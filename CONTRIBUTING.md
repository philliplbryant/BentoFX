...
- Because they block or modify external resources, integration tests <u>cannot</u> be run reliably in parallel.
...
- May interact with external resources but do not lock or modify them and allow other tests to concurrently access, but not modify, the same resources.
- Because they do not modify external resources, integration tests parallel can be run reliably in parallel.
...
- Often interact with user interface components and therefore cannot be run in parallel.
