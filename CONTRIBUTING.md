...
## Code Coverage and Aggregated Reports

BentoFX keeps normal development builds fast by running tests without collecting coverage unless coverage is explicitly requested.

Use the normal lifecycle tasks for local development:
...
Use the CI-style command when you want aggregated test reports and JaCoCo coverage reports:

```shell
gradlew build buildHealth checkAll -PcollectCoverage=true -PtestReportMode=ci
```

The `collectCoverage` property controls JaCoCo execution and report generation:

- Omit `-PcollectCoverage=true` for normal local builds.
- Add `-PcollectCoverage=true` when coverage data and reports are required.

The `testReportMode` property controls aggregate test report lifecycle wiring:

- `-PtestReportMode=ci` wires aggregate reports into the verification lifecycle.
- Omitting the property keeps local builds focused on executing tests rather than generating aggregate reports.

JaCoCo reports are available per project and from the report aggregation project. The aggregate reports are generated by [report-aggregation](./report-aggregation/build.gradle) for library modules only. Demo projects and `persistence:test-fixtures` are intentionally excluded from aggregate coverage.

Expected aggregate report locations include:

- `report-aggregation/build/reports/tests/test/aggregated-results/index.html`
- `report-aggregation/build/reports/tests/integrationTest/aggregated-results/index.html`
- `report-aggregation/build/reports/tests/integrationTestParallel/aggregated-results/index.html`
- `report-aggregation/build/reports/tests/functionalTest/aggregated-results/index.html`
- `report-aggregation/build/reports/jacoco/testCodeCoverageReport/html/index.html`
- `report-aggregation/build/reports/jacoco/integrationTestCodeCoverageReport/html/index.html`
- `report-aggregation/build/reports/jacoco/integrationTestParallelCodeCoverageReport/html/index.html`
- `report-aggregation/build/reports/jacoco/functionalTestCodeCoverageReport/html/index.html`

The GitHub workflow runs the CI-style command above, uploads generated JaCoCo and test reports as workflow artifacts, and writes source-line and coverage statistics to the workflow summary.
